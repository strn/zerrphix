...
class ArtworkCollection(ArtworkBase):
    """The Film Artwork Class which renders iamges for use on the Dune UI
    """

    def __init__(self, **kwargs):
        super(ArtworkCollection, self).__init__(**kwargs)
        """Artwork __init__
        ...
        """
        self.smbcon = None
        self.specials = film_collection_specials.Specials(self.Session, self.library_config_dict['name'])
    ...
    def render(self):
        ...
        # print(ZP_FILM_COLLECTION_ID)
        # raise SystemExit
        # ZP_FILM_COLLECTION_ID = 0
    ...
    def get_entity_processing_dict(self, **kwargs):
        ...
        qry_films_with_eid = session.query(
            ...
            ~TABLES.ZP_FILM_COLLECTION.ID.in_(
                session.query(
                    TABLES.ZP_USER_FILM_COLLECTION_IMAGE_RENDER_HASH_XREF.ZP_FILM_COLLECTION_ID).filter(
                    ...
                    TABLES.ZP_USER_FILM_COLLECTION_IMAGE_RENDER_HASH_XREF.ZP_IMAGE_SUB_TYPE == kwargs['ZP_IMAGE_SUB_TYPE'],
                    ...
                    #TABLES.ZP_FILM_COLLECTION_IMAGE_RENDER_HASH.TEMPLATE_NAME == kwargs['template_name'],
                    #TABLES.ZP_FILM_COLLECTION_IMAGE_RENDER_HASH.TEMPLATE_VERSION == kwargs['template_version'],
                    TABLES.ZP_FILM_COLLECTION_IMAGE_RENDER_HASH.ZP_TEMPLATE_ID == kwargs['zp_template_id'],
                    TABLES.ZP_FILM_COLLECTION_IMAGE_RENDER_HASH.ZP_TEMPLATE_ID == TABLES.ZP_TEMPLATE.ID,
                    TABLES.ZP_DUNE_FILM_COLLECTION_IMAGE_RENDER_HASH_XREF.RENDER_DATETIME >=
                    ...
                    TABLES.ZP_FILM_COLLECTION.ID == TABLES.ZP_FILM_COLLECTION_IMAGE_RENDER_HASH.ZP_FILM_COLLECTION_ID))
        )
        ...
        if qry_films_with_eid_count > 0:
            films = qry_films_with_eid.order_by(TABLES.ZP_FILM_COLLECTION.ID.desc()).limit(100)
    ...
    def porcess_entity(self, **kwargs):
        ...
        """
        #kwargs['ZP_FILM_COLLECTION_IMAGE_TYPE_ID'] = self.image_type_dict[kwargs['image_type']]
        session = self.Session()
        max_film_id = session.query(func.max(TABLES.ZP_FILM.ID)).one()[0]
        session.close()
        if isinstance(max_film_id, int):
            for ZP_IMAGE_SUB_TYPE in kwargs['icon_sub_type_require_list']:
                ...
                kwargs['ZP_FILM_COLLECTION_ID'] = ZP_FILM_COLLECTION_ID
                    ...
                    if entity_processing_list:
                        for ID in entity_processing_list:
                            ...
                            if image_ident is not None:
                                ...
                                #log.warning('c %s', image_ident)
                                kwargs['image_ident_md5'] = md5(image_ident).hexdigest()
                               #log.warning('c %s', kwargs['image_ident_md5'])
                                kwargs['ZP_FILM_COLLECTION_IMAGE_RENDER_HASH_ID'] = self.get_image_hash(**kwargs)
                                if kwargs['ZP_FILM_COLLECTION_IMAGE_RENDER_HASH_ID'] is not None:
                                    # log.warning('found ZP_FILM_EPISODE_IMAGE_RENDER_HASH_ID %s for ZP_DUNE_ID %s, ZP_FILM_COLLECTION_ID %s, SEASON %s, FIRST_EPISODE %s, ZP_FILM_COLLECTION_IMAGE_TYPE_ID %s, ZP_IMAGE_SUB_TYPE %s,'
                                    #			'image_ident_md5 %s, template_name %s, template_version %s',
                                    #			ZP_FILM_EPISODE_IMAGE_RENDER_HASH_ID, ZP_DUNE_ID, ZP_FILM_COLLECTION_ID, SEASON, FIRST_EPISODE, ZP_FILM_COLLECTION_IMAGE_TYPE_ID, ZP_IMAGE_SUB_TYPE,
                                    #			image_ident_md5, template_name, template_version)
                                    self.set_user_image_hash(**kwargs)
                                    # log.warning('set user xref ZP_FILM_EPISODE_IMAGE_RENDER_HASH_ID %s for ZP_FILM_COLLECTION_ID %s, SEASON %s, FIRST_EPISODE %s, ZP_FILM_COLLECTION_IMAGE_TYPE_ID %s, ZP_IMAGE_SUB_TYPE %s,'
                                    #		'image_ident_md5 %s, template_name %s, template_version %s',
                                    #		ZP_FILM_EPISODE_IMAGE_RENDER_HASH_ID, ZP_FILM_COLLECTION_ID, SEASON, FIRST_EPISODE, ZP_FILM_COLLECTION_IMAGE_TYPE_ID, ZP_IMAGE_SUB_TYPE,
                                    #		image_ident_md5, template_name, template_version)
                                    render_image = False
                                    # raise SystemExit
                            if render_image:
                                if 'ZP_FILM_COLLECTION_IMAGE_RENDER_HASH_ID' in kwargs:
                                    del kwargs['ZP_FILM_COLLECTION_IMAGE_RENDER_HASH_ID']
    ...
    def render_image(self, **kwargs):
        ...
        log.debug('image_template_folder %s', image_template_folder)
        image_save_folder = os.path.join(image_template_folder, str(kwargs['ZP_FILM_COLLECTION_ID']))
        ...
        if image_save_folder_exists:
            ...
            for ZP_FILM_COLLECTION_SPECIAL_TYPE_ID in sorted(specials_dict):
                # if ZP_FILM_COLLECTION_SPECIAL_TYPE_ID in [2]:
                # TODO: Not using lang here as should be same title accross all langs? change to use lang
                # image_ident += '{0}'.format(specials_dict[ZP_FILM_COLLECTION_SPECIAL_TYPE_ID]['ZP_FILM_TITLE_ID'])
                # print(specials_dict)
                if ZP_FILM_COLLECTION_SPECIAL_TYPE_ID in [2]:
                    # TODO: Not using lang here as user defined title by user prefs class
                    image_ident += '{0}'.format(specials_dict[ZP_FILM_COLLECTION_SPECIAL_TYPE_ID]['title'])
                    log.debug('special %s, %s', ZP_FILM_COLLECTION_SPECIAL_TYPE_ID,
                              specials_dict[ZP_FILM_COLLECTION_SPECIAL_TYPE_ID]['title'])
                elif ZP_FILM_COLLECTION_SPECIAL_TYPE_ID in [3]:
                    # TODO: Not using lang here as user defined title by user prefs class
                    image_ident += '{0}'.format(specials_dict[ZP_FILM_COLLECTION_SPECIAL_TYPE_ID]['overview'])
                    log.debug('special %s, %s', ZP_FILM_COLLECTION_SPECIAL_TYPE_ID,
                              specials_dict[ZP_FILM_COLLECTION_SPECIAL_TYPE_ID]['overview'])
                elif ZP_FILM_COLLECTION_SPECIAL_TYPE_ID in [1,4]:
                    image_ident += '{0}'.format(specials_dict[ZP_FILM_COLLECTION_SPECIAL_TYPE_ID]['ZP_RAW_IMAGE_ID'])
                    log.debug('special %s, %s', ZP_FILM_COLLECTION_SPECIAL_TYPE_ID,
                              specials_dict[ZP_FILM_COLLECTION_SPECIAL_TYPE_ID]['ZP_RAW_IMAGE_ID'])
                elif self.special_type_dict_by_id[ZP_FILM_COLLECTION_SPECIAL_TYPE_ID]['USES_LANG'] == 1:
                    # print(ZP_FILM_COLLECTION_SPECIAL_TYPE_ID, specials_dict[ZP_FILM_COLLECTION_SPECIAL_TYPE_ID])
                    image_ident += '{0}{1}{2}'.format(ZP_FILM_COLLECTION_SPECIAL_TYPE_ID,
                                                      specials_dict[ZP_FILM_COLLECTION_SPECIAL_TYPE_ID][
                                                          'ZP_EAPI_ID_ACT'],
                                                      specials_dict[ZP_FILM_COLLECTION_SPECIAL_TYPE_ID][
                                                          'ZP_LANG_ID_ACT'])
                    self.specials.eapi_special_update_db(kwargs['ZP_FILM_COLLECTION_ID'],
                    ...
                    image_ident += '{0}{1}'.format(ZP_FILM_COLLECTION_SPECIAL_TYPE_ID,
                                                   specials_dict[ZP_FILM_COLLECTION_SPECIAL_TYPE_ID][
                                                       'ZP_EAPI_ID_ACT'])
                    self.specials.eapi_special_update_db(kwargs['ZP_FILM_COLLECTION_ID'],
                                                         ZP_FILM_COLLECTION_SPECIAL_TYPE_ID,
                                                         ...
                                                         specials_dict[ZP_FILM_COLLECTION_SPECIAL_TYPE_ID][
                                                             'ZP_EAPI_ID_ACT'],
                                                         kwargs['zp_image_type_id'])
           # log.warning('r %s', image_ident)
            image_ident_md5 = md5(image_ident).hexdigest()
            kwargs['image_ident_md5'] = image_ident_md5
           # log.warning('r %s', image_ident_md5)
            kwargs['image_save_path'] += '{0}.{1}'.format(image_ident_md5, kwargs['image_extension'])
            log.debug(
                'rendering image for ZP_FILM_COLLECTION_ID: {0}, ZP_DUNE_ID: {1}, ZP_USER_ID: {2}, ZP_IMAGE_SUB_TYPE: {3},'
                ' self.image_type_dict[image_type]: {4}, image_ident_md5: {5},'
                ' template_name: {6}, template_version: {7}'.format(kwargs['ZP_FILM_COLLECTION_ID'],
            ...
            if save_image(img, kwargs['image_extension'], smbcon=self.smbcon, **kwargs):
                ...
                # log.warning('set or updated image hash for ZP_FILM_COLLECTION_ID %s, SEASON %s, FIRST_EPISODE %s, ZP_FILM_COLLECTION_IMAGE_TYPE_ID %s, ZP_IMAGE_SUB_TYPE %s,'
                #		'image_ident_md5 %s, template_name %s, template_version %s',
                #		ZP_FILM_COLLECTION_ID, SEASON, FIRST_EPISODE, ZP_FILM_COLLECTION_IMAGE_TYPE_ID, ZP_IMAGE_SUB_TYPE,
                #		image_ident_md5, template_name, template_version)
                # raise SystemExit
    ...
    def get_image_hash(self, **kwargs):
        session = self.Session()
        try:
            ZP_FILM_COLLECTION_IMAGE_RENDER_HASH = session.query(TABLES.ZP_FILM_COLLECTION_IMAGE_RENDER_HASH).filter(
                TABLES.ZP_FILM_COLLECTION_IMAGE_RENDER_HASH.ZP_FILM_COLLECTION_ID == kwargs['ZP_FILM_COLLECTION_ID'],
                TABLES.ZP_FILM_COLLECTION_IMAGE_RENDER_HASH.ZP_IMAGE_SUB_TYPE == kwargs['ZP_IMAGE_SUB_TYPE'],
                TABLES.ZP_FILM_COLLECTION_IMAGE_RENDER_HASH.ZP_FILM_COLLECTION_IMAGE_TYPE_ID ==
                kwargs['zp_image_type_id'],
                TABLES.ZP_FILM_COLLECTION_IMAGE_RENDER_HASH.HASH == kwargs['image_ident_md5'],
                ##
            ...
            ZP_FILM_COLLECTION_IMAGE_RENDER_HASH_ID = None
        else:
            ZP_FILM_COLLECTION_IMAGE_RENDER_HASH_ID = ZP_FILM_COLLECTION_IMAGE_RENDER_HASH.ID
        session.close()
        return ZP_FILM_COLLECTION_IMAGE_RENDER_HASH_ID

    def set_dune_image_hash(self, **kwargs):
        session = self.Session()
        try:
            ZP_DUNE_FILM_COLLECTION_IMAGE_RENDER_HASH_XREF = session.query(
                TABLES.ZP_DUNE_FILM_COLLECTION_IMAGE_RENDER_HASH_XREF
            ).filter(
                TABLES.ZP_DUNE_FILM_COLLECTION_IMAGE_RENDER_HASH_XREF.ZP_DUNE_ID == kwargs['ZP_DUNE_ID'],
                TABLES.ZP_DUNE_FILM_COLLECTION_IMAGE_RENDER_HASH_XREF.ZP_FILM_COLLECTION_IMAGE_RENDER_HASH_ID ==
                kwargs['ZP_FILM_COLLECTION_IMAGE_RENDER_HASH_ID']
            ).one()
        except orm.exc.NoResultFound:
            log.debug(('Adding ZP_DUNE_ID = {0},'
                       ' ZP_FILM_COLLECTION_IMAGE_RENDER_HASH_ID = {1} to'
                       ' ZP_DUNE_FILM_COLLECTION_IMAGE_RENDER_HASH_XREF').format(kwargs['ZP_DUNE_ID'],
                                                                      kwargs['ZP_FILM_COLLECTION_IMAGE_RENDER_HASH_ID']))
            add_ZP_DUNE_FILM_COLLECTION_IMAGE_RENDER_HASH_XREF = TABLES.ZP_DUNE_FILM_COLLECTION_IMAGE_RENDER_HASH_XREF(
                ZP_DUNE_ID=kwargs['ZP_DUNE_ID'],
                ZP_FILM_COLLECTION_IMAGE_RENDER_HASH_ID=kwargs['ZP_FILM_COLLECTION_IMAGE_RENDER_HASH_ID'],
                RENDER_DATETIME=date_time(),
                EXT=kwargs['image_extension']
            )
            session.add(add_ZP_DUNE_FILM_COLLECTION_IMAGE_RENDER_HASH_XREF)
            commit(session)
        else:
            ZP_DUNE_FILM_COLLECTION_IMAGE_RENDER_HASH_XREF.RENDER_DATETIME = date_time()
            ZP_DUNE_FILM_COLLECTION_IMAGE_RENDER_HASH_XREF.EXT = kwargs['image_extension']
            commit(session)
        session.close()
    ...
    def set_user_image_hash(self, **kwargs):
        """Set the hash used for the user in the filename in the db
            ...
            Args:
                | ZP_USER_ID (int): the user id
                | ZP_FILM_COLLECTION_ID (int): the tv id
                | ZP_FILM_COLLECTION_IMAGE_TYPE_ID (int): tv image type (icon/synopsis) id
                | image_ident_md5 (string): md5 hash used in the filename of the image
                | template_name (string): template name
                | template_version (string): template version

            Raises:
                Exception: if cannot find the hash in ZP_FILM_COLLECTION_IMAGE_RENDER_HASH as it should be in there

        """
        session = self.Session()
        try:
            ZP_USER_FILM_COLLECTION_IMAGE_RENDER_HASH_XREF = session.query(
                TABLES.ZP_USER_FILM_COLLECTION_IMAGE_RENDER_HASH_XREF).filter(
                TABLES.ZP_USER_FILM_COLLECTION_IMAGE_RENDER_HASH_XREF.ZP_FILM_COLLECTION_ID == kwargs['ZP_FILM_COLLECTION_ID'],
                TABLES.ZP_USER_FILM_COLLECTION_IMAGE_RENDER_HASH_XREF.ZP_USER_ID == kwargs['ZP_USER_ID'],
                TABLES.ZP_USER_FILM_COLLECTION_IMAGE_RENDER_HASH_XREF.ZP_FILM_COLLECTION_IMAGE_TYPE_ID == kwargs['zp_image_type_id'],
                TABLES.ZP_USER_FILM_COLLECTION_IMAGE_RENDER_HASH_XREF.ZP_IMAGE_SUB_TYPE == kwargs['ZP_IMAGE_SUB_TYPE']
            ).one()
        except orm.exc.NoResultFound as e:
            log.debug(('Adding ZP_FILM_COLLECTION_ID = {0}').format(kwargs['ZP_FILM_COLLECTION_ID']))
            add_ZP_USER_FILM_COLLECTION_IMAGE_RENDER_HASH_XREF = TABLES.ZP_USER_FILM_COLLECTION_IMAGE_RENDER_HASH_XREF(
                ZP_FILM_COLLECTION_ID=kwargs['ZP_FILM_COLLECTION_ID'],
                ZP_USER_ID=kwargs['ZP_USER_ID'],
                ZP_FILM_COLLECTION_IMAGE_TYPE_ID=kwargs['zp_image_type_id'],
                ZP_IMAGE_SUB_TYPE=kwargs['ZP_IMAGE_SUB_TYPE'],
                ZP_FILM_COLLECTION_IMAGE_RENDER_HASH_ID=kwargs['ZP_FILM_COLLECTION_IMAGE_RENDER_HASH_ID'])
            session.add(add_ZP_USER_FILM_COLLECTION_IMAGE_RENDER_HASH_XREF)
            commit(session)
        else:
            if ZP_USER_FILM_COLLECTION_IMAGE_RENDER_HASH_XREF.ZP_FILM_COLLECTION_IMAGE_RENDER_HASH_ID != kwargs[
                'ZP_FILM_COLLECTION_IMAGE_RENDER_HASH_ID']:
                ZP_USER_FILM_COLLECTION_IMAGE_RENDER_HASH_XREF.ZP_FILM_COLLECTION_IMAGE_RENDER_HASH_ID = kwargs[
                    'ZP_FILM_COLLECTION_IMAGE_RENDER_HASH_ID']
                commit(session)
        session.close()
    ...
    def set_image_hash(self, **kwargs):
        ...
        try:
            ZP_FILM_COLLECTION_IMAGE_RENDER_HASH = session.query(TABLES.ZP_FILM_COLLECTION_IMAGE_RENDER_HASH).filter(
                TABLES.ZP_FILM_COLLECTION_IMAGE_RENDER_HASH.ZP_FILM_COLLECTION_ID == kwargs['ZP_FILM_COLLECTION_ID'],
                TABLES.ZP_FILM_COLLECTION_IMAGE_RENDER_HASH.ZP_IMAGE_SUB_TYPE == kwargs['ZP_IMAGE_SUB_TYPE'],
                TABLES.ZP_FILM_COLLECTION_IMAGE_RENDER_HASH.ZP_FILM_COLLECTION_IMAGE_TYPE_ID == kwargs['zp_image_type_id'],
                TABLES.ZP_FILM_COLLECTION_IMAGE_RENDER_HASH.HASH == kwargs['image_ident_md5'],
                # TABLES.ZP_FILM_COLLECTION_IMAGE_RENDER_HASH.TEMPLATE_NAME == kwargs['template_name'],
                # TABLES.ZP_FILM_COLLECTION_IMAGE_RENDER_HASH.TEMPLATE_VERSION == kwargs['template_version']
                TABLES.ZP_FILM_COLLECTION_IMAGE_RENDER_HASH.ZP_TEMPLATE_ID == kwargs['zp_template_id']
            ...
            log.debug(('Adding ZP_FILM_COLLECTION_ID = {0}, image_ident_md5 = {1}').format(
                kwargs['ZP_FILM_COLLECTION_ID'], kwargs['image_ident_md5']))
            add_ZP_FILM_COLLECTION_IMAGE_RENDER_HASH = TABLES.ZP_FILM_COLLECTION_IMAGE_RENDER_HASH(
                ZP_FILM_COLLECTION_ID=kwargs['ZP_FILM_COLLECTION_ID'],
                ZP_IMAGE_SUB_TYPE=kwargs['ZP_IMAGE_SUB_TYPE'],
                ZP_FILM_COLLECTION_IMAGE_TYPE_ID=kwargs['zp_image_type_id'],
            ...
            session.add(add_ZP_FILM_COLLECTION_IMAGE_RENDER_HASH)
            commit(session)
            kwargs['ZP_FILM_COLLECTION_IMAGE_RENDER_HASH_ID'] = add_ZP_FILM_COLLECTION_IMAGE_RENDER_HASH.ID
        else:
            kwargs['ZP_FILM_COLLECTION_IMAGE_RENDER_HASH_ID'] = ZP_FILM_COLLECTION_IMAGE_RENDER_HASH.ID
            # ZP_FILM_COLLECTION_IMAGE_RENDER_HASH.EXT = kwargs['image_extension']
            commit(session)

            log.debug(('ZP_FILM_COLLECTION_ID: {0},'
                       ' image_ident_md5: {1} allready in ZP_FILM_COLLECTION_IMAGE_RENDER_HASH'.format(kwargs['ZP_FILM_COLLECTION_ID'],
                                                                                            kwargs['image_ident_md5'])))
        session.close()
        ...
        for eapi in self.eapi_plugins_access_list:
            ...
            try:
                zp_eapi_eid = session.query(
                    TABLES.ZP_FILM_COLLECTION_EAPI_EID).filter(
                    TABLES.ZP_FILM_COLLECTION_EAPI_EID.ZP_FILM_COLLECTION_ID == kwargs['ZP_FILM_COLLECTION_ID'],
                    TABLES.ZP_FILM_COLLECTION_EAPI_EID.ZP_EAPI_ID == self.eapi_dict[eapi]).one().ZP_EAPI_COLLECTION_EID
            except orm.exc.NoResultFound as e:
                log.warning('No resluts for ZP_FILM_COLLECTION_ID: {0} in ZP_FILM_COLLECTION_EAPI_EID'.format(
    ...
    def get_resized_raw_image(self, speical_image_type, width, height, **kwargs):
        ...
        try:
            zp_film_raw_image_xref = session.query(TABLES.ZP_FILM_COLLECTION_RAW_IMAGE).filter(
                ...
                TABLES.ZP_USER_FILM_COLLECTION_ENTITY_XREF.ZP_FILM_COLLECTION_ENTITY_TYPE_ID == speical_image_type_entity_type_dict[
                    speical_image_type]
            ).one()
        except orm.exc.NoResultFound:
            ...
            image_path = os.path.join('%s_collection' % self.library_config_dict['downloaded_images_library_root_path'],
