...
#from zerrphix.specials import user_specials
from zerrphix.specials import user_specials
...
class ArtworkUser(ArtworkBase):
    ...
    def __init__(self, **kwargs):
        ...
        """
        self.smbcon = None
        self.specials = user_specials.Specials(self.Session, self.library_config_dict['name'])
        ...
        SELECT ZP_PEOPLE.*, (SELECT COUNT(*) FROM ZP_USER_ROLE_XREF WHERE ZP_USER_ROLE_XREF.ZP_PEOPLE_ID = ZP_PEOPLE.ID) AS TOT
        FROM ZP_PEOPLE
        where ID in (SELECT ZP_PEOPLE_ID FROM ZP_USER_ROLE_XREF WHERE ZP_USER_ID = 1)
        ORDER BY TOT DESC
        """
    ...
    # count_poeple_occourance = session.query(func.count(TABLES.ZP_USER_ROLE_XREF)).filter(TABLES.ZP_USER_ROLE_XREF.ZP_PEOPLE_ID == TABLES.ZP_PEOPLE.ID).subquery()
    # count_poeple_occourance = session.query(func.count('*')).filter(
    #							TABLES.ZP_USER_ROLE_XREF.ZP_PEOPLE_ID == TABLES.ZP_PEOPLE.ID).subquery()
    # directors = session.query(
    #					TABLES.ZP_PEOPLE.NAME, count_poeple_occourance).filter(
    #						TABLES.ZP_PEOPLE.ID.in_(
    #							session.query(
    #								TABLES.ZP_USER_ROLE_XREF.ZP_PEOPLE_ID).filter(
    #									TABLES.ZP_USER_ROLE_XREF.ZP_ROLE_ID == 1,
    #									TABLES.ZP_USER_ROLE_XREF.ZP_USER_ID == 1))).all()
    ...
    def render(self):
        ...
        max_film_id = session.query(func.max(TABLES.ZP_USER.ID)).one()[0]
        session.close()
        # print(max_film_id)
        ...
        # raise SystemExit
            ...
            log.warning('There do not seem to be any films in TABLES.ZP_USER')

    def get_entity_processing_dict(self, **kwargs):
        session = self.Session()
        # session.refresh(TABLES.ZP_USER)
        # session.expire_on_commit = False
        ...
        qry_films_with_eid = session.query(
            ...
            TABLES.ZP_USER.ID == kwargs['ZP_USER_ID'],
            # make this optional ???
            ~TABLES.ZP_USER.ID.in_(
                session.query(
                    TABLES.ZP_USER_IMAGE_RENDER_HASH.ZP_USER_ID).filter(
                    TABLES.ZP_USER_IMAGE_RENDER_HASH.ZP_USER_ID == kwargs['ZP_USER_ID'],
                    TABLES.ZP_USER_IMAGE_RENDER_HASH.ZP_USER_ID == TABLES.ZP_USER.ID,
                    TABLES.ZP_USER_IMAGE_RENDER_HASH.ZP_USER_IMAGE_TYPE_ID == kwargs['zp_image_type_id'],
                    TABLES.ZP_USER_IMAGE_RENDER_HASH.TEMPLATE_NAME == kwargs['template_name'],
                    TABLES.ZP_DUNE_USER_IMAGE_RENDER_HASH_XREF.RENDER_DATETIME >=
                    TABLES.ZP_USER.LAST_EDIT_DATETIME,
                    TABLES.ZP_USER_IMAGE_RENDER_HASH.ID ==
                    TABLES.ZP_DUNE_USER_IMAGE_RENDER_HASH_XREF.ZP_USER_IMAGE_RENDER_HASH_ID,
                    TABLES.ZP_DUNE_USER_IMAGE_RENDER_HASH_XREF.ZP_DUNE_ID == kwargs['ZP_DUNE_ID'],
                    TABLES.ZP_USER_IMAGE_RENDER_HASH.ZP_IMAGE_SUB_TYPE == kwargs['ZP_IMAGE_SUB_TYPE']))
        )
    ...
    def porcess_entity(self, **kwargs):
        ...
        """
        #kwargs['ZP_USER_IMAGE_TYPE_ID'] # = self.image_type_dict[kwargs['image_type']]
        #session = self.Session()
        #max_user_id = session.query(func.max(TABLES.ZP_USER.ID)).one()[0]
        #session.close()
        #if isinstance(max_film_id, int):
        if 1 == 1:
            for ZP_IMAGE_SUB_TYPE in kwargs['icon_sub_type_require_list']:
                ...
                while entities_processed is False:
                    ...
                    if entity_processing_list is True:
                        #for ID in entity_processing_list:
                        if 1 == 1:
                            log.debug('dune_id %s, userid %s image_type %s, ZP_IMAGE_SUB_TYPE %s',
                                      kwargs['ZP_DUNE_ID'],
                                      kwargs['ZP_USER_ID'],
                                      kwargs['image_type'],
                                      ZP_IMAGE_SUB_TYPE)
                            #kwargs['ZP_USER_ID'] = ID
                            #kwargs['ZP_ENTITY_ID'] = ID
                            self.set_current_library_process_desc_detail(
                                self.library_config_dict['id'],
                                 7,
                                 'User %s, Image_type %s, sub_type %s, Dune %s' % (
                            ...
                            image_ident = str(kwargs['ZP_USER_ID'])
                            ...
                            if image_ident is not None:
                                ...
                                #image_ident = '{0}T{1}'.format(kwargs['ZP_USER_IMAGE_TYPE_ID'], image_ident)
                                #image_ident =
                                ...
                                kwargs['ZP_USER_IMAGE_RENDER_HASH_ID'] = self.get_image_hash(**kwargs)                                
                                if kwargs['ZP_USER_IMAGE_RENDER_HASH_ID'] is not None:
                                    # log.warning('found ZP_USER_EPISODE_IMAGE_RENDER_HASH_ID %s for ZP_DUNE_ID %s, ZP_USER_ID %s, SEASON %s, FIRST_EPISODE %s, ZP_USER_IMAGE_TYPE_ID %s, ZP_IMAGE_SUB_TYPE %s,'
                                    #			'image_ident_md5 %s, template_name %s, template_version %s',
                                    #			ZP_USER_EPISODE_IMAGE_RENDER_HASH_ID, ZP_DUNE_ID, ZP_USER_ID, SEASON, FIRST_EPISODE, ZP_USER_IMAGE_TYPE_ID, ZP_IMAGE_SUB_TYPE,
                                    #			image_ident_md5, template_name, template_version)
                                    #self.set_user_image_hash(**kwargs)
                                    # log.warning('set user xref ZP_USER_EPISODE_IMAGE_RENDER_HASH_ID %s for ZP_USER_ID %s, SEASON %s, FIRST_EPISODE %s, ZP_USER_IMAGE_TYPE_ID %s, ZP_IMAGE_SUB_TYPE %s,'
                                    #		'image_ident_md5 %s, template_name %s, template_version %s',
                                    #		ZP_USER_EPISODE_IMAGE_RENDER_HASH_ID, ZP_USER_ID, SEASON, FIRST_EPISODE, ZP_USER_IMAGE_TYPE_ID, ZP_IMAGE_SUB_TYPE,
                                    #		image_ident_md5, template_name, template_version)
                                    render_image = False
                                    # raise SystemExit
                            else:
                                log.error('image_ident is none for userid %s',
                                          kwargs['ZP_USER_ID']
                                          )
                            if render_image:
                                if 'ZP_USER_IMAGE_RENDER_HASH_ID' in kwargs:
                                    del kwargs['ZP_USER_IMAGE_RENDER_HASH_ID']
    ...
    def render_image(self, **kwargs):
        log.debug('rendering ZP_DUNE_ID %s, ZP_USER_ID %s, image_type %s, ZP_IMAGE_SUB_TYPE %s',
                  kwargs['ZP_DUNE_ID'], kwargs['ZP_USER_ID'], kwargs['image_type'],
                  kwargs['ZP_IMAGE_SUB_TYPE'])
        image_save_folder_exists = False
        image_save_folder = os.path.join(kwargs['dune_render_store_dict']['rendered_user_image_root_path'])
        ...
        if image_save_folder_exists:
            kwargs['image_save_path'] = os.path.join(image_save_folder,
                                                     '{}.{}{}.'.format(str(kwargs['ZP_USER_ID']),
                                                         kwargs['image_type'],
                                                                       self.image_sub_type_dict_by_id[
                                                                           kwargs['ZP_IMAGE_SUB_TYPE']]))
            log.debug('image_save_path: {0}'.format(kwargs['image_save_path']))
            # raise SystemExit
            #image_ident = '{0}T'.format(self.image_type_dict[kwargs['image_type']])
            image_ident = str(kwargs['ZP_USER_ID'])
            ...
            #for ZP_USER_SPECIAL_TYPE_ID in sorted(specials_dict):
            #    # if ZP_USER_SPECIAL_TYPE_ID in [2]:
            #    # TODO: Not using lang here as should be same title accross all langs? change to use lang
            #    # image_ident += '{0}'.format(specials_dict[ZP_USER_SPECIAL_TYPE_ID]['ZP_USER_TITLE_ID'])
            #    # print(specials_dict)
            #    log.debug(specials_dict)
            #    if ZP_USER_SPECIAL_TYPE_ID in [4]:
            #        # TODO: Not using lang here as user defined title by user prefs class
            #        image_ident += '{0}'.format(specials_dict[ZP_USER_SPECIAL_TYPE_ID]['title'])
            #        log.debug('special %s, %s', ZP_USER_SPECIAL_TYPE_ID,
            #                  specials_dict[ZP_USER_SPECIAL_TYPE_ID]['title'])
            #    elif ZP_USER_SPECIAL_TYPE_ID in [10]:
            #        # TODO: Not using lang here as user defined title by user prefs class
            #        image_ident += '{0}'.format(specials_dict[ZP_USER_SPECIAL_TYPE_ID]['overview'])
            #        log.debug('special %s, %s', ZP_USER_SPECIAL_TYPE_ID,
            #                  specials_dict[ZP_USER_SPECIAL_TYPE_ID]['overview'])
            #    elif ZP_USER_SPECIAL_TYPE_ID in [1,2]:
            #        image_ident += '{0}'.format(specials_dict[ZP_USER_SPECIAL_TYPE_ID]['ZP_RAW_IMAGE_ID'])
            #        log.debug('special %s, %s', ZP_USER_SPECIAL_TYPE_ID,
            #                  specials_dict[ZP_USER_SPECIAL_TYPE_ID]['ZP_RAW_IMAGE_ID'])
            #    elif self.special_type_dict_by_id[ZP_USER_SPECIAL_TYPE_ID]['USES_LANG'] == 1:
            #        # print(ZP_USER_SPECIAL_TYPE_ID, specials_dict[ZP_USER_SPECIAL_TYPE_ID])
            #        image_ident += '{0}{1}{2}'.format(ZP_USER_SPECIAL_TYPE_ID,
            #                                          specials_dict[ZP_USER_SPECIAL_TYPE_ID][
            #                                              'ZP_EAPI_ID_ACT'],
            #                                          specials_dict[ZP_USER_SPECIAL_TYPE_ID][
            #                                              'ZP_LANG_ID_ACT'])
            #        log.debug('special %s, %s', ZP_USER_SPECIAL_TYPE_ID,
            #                  '{0}{1}{2}'.format(ZP_USER_SPECIAL_TYPE_ID,
            #                                     specials_dict[ZP_USER_SPECIAL_TYPE_ID][
            #                                         'ZP_EAPI_ID_ACT'],
            #                                     specials_dict[ZP_USER_SPECIAL_TYPE_ID][
            #                                         'ZP_LANG_ID_ACT']))
            #        self.specials.eapi_special_update_db(kwargs['ZP_USER_ID'],
            #                                             ZP_USER_SPECIAL_TYPE_ID,
            #                                             specials_dict[ZP_USER_SPECIAL_TYPE_ID][
            #                                                 'ZP_LANG_ID_REQ'],
            #                                             specials_dict[ZP_USER_SPECIAL_TYPE_ID][
            #                                                 'ZP_LANG_ID_ACT'],
            ...
            #                                             specials_dict[ZP_USER_SPECIAL_TYPE_ID][
            #                                                 'ZP_EAPI_ID_ACT'],
            #                                             kwargs['ZP_USER_IMAGE_TYPE_ID'])
            #    else:
            #        image_ident += '{0}{1}'.format(ZP_USER_SPECIAL_TYPE_ID,
            #                                       specials_dict[ZP_USER_SPECIAL_TYPE_ID][
            #                                           'ZP_EAPI_ID_ACT'])
            #        log.debug('special %s, %s', ZP_USER_SPECIAL_TYPE_ID,
            #                  '{0}{1}'.format(ZP_USER_SPECIAL_TYPE_ID,
            #                                  specials_dict[ZP_USER_SPECIAL_TYPE_ID][
            #                                      'ZP_EAPI_ID_ACT']))
            #        self.specials.eapi_special_update_db(kwargs['ZP_USER_ID'],
            #                                             ZP_USER_SPECIAL_TYPE_ID,
            ...
            log.debug('save_path: %s', kwargs['image_save_path'])
            log.debug(
                'rendering image for ZP_USER_ID: {0}, ZP_DUNE_ID: {1}, ZP_USER_ID: {2}, ZP_IMAGE_SUB_TYPE: {3},'
                ' self.image_type_dict[image_type]: {4}, image_ident_md5: {5},'
                ' template_name: {6}, template_version: {7}'.format(kwargs['ZP_USER_ID'],
            ...
            if save_image(img, kwargs['image_extension'], smbcon=self.smbcon, **kwargs):
                ...
                # log.warning('set or updated image hash for ZP_USER_ID %s, SEASON %s, FIRST_EPISODE %s, ZP_USER_IMAGE_TYPE_ID %s, ZP_IMAGE_SUB_TYPE %s,'
                #		'image_ident_md5 %s, template_name %s, template_version %s',
                #		ZP_USER_ID, SEASON, FIRST_EPISODE, ZP_USER_IMAGE_TYPE_ID, ZP_IMAGE_SUB_TYPE,
                #		image_ident_md5, template_name, template_version)
                # raise SystemExit
    ...
    def get_image_hash(self, **kwargs):
        session = self.Session()
        try:
            ZP_USER_IMAGE_RENDER_HASH = session.query(TABLES.ZP_USER_IMAGE_RENDER_HASH).filter(
                TABLES.ZP_USER_IMAGE_RENDER_HASH.ZP_IMAGE_SUB_TYPE == kwargs['ZP_IMAGE_SUB_TYPE'],
                TABLES.ZP_USER_IMAGE_RENDER_HASH.ZP_USER_IMAGE_TYPE_ID == kwargs['zp_image_type_id'],
                TABLES.ZP_USER_IMAGE_RENDER_HASH.HASH == kwargs['image_ident_md5'],
                TABLES.ZP_USER_IMAGE_RENDER_HASH.TEMPLATE_NAME == kwargs['template_name'],
                TABLES.ZP_USER.ID == TABLES.ZP_USER_IMAGE_RENDER_HASH.ZP_USER_ID,
                TABLES.ZP_DUNE_USER_IMAGE_RENDER_HASH_XREF.ZP_DUNE_ID == kwargs['ZP_DUNE_ID'],
                TABLES.ZP_DUNE_USER_IMAGE_RENDER_HASH_XREF.ZP_USER_IMAGE_RENDER_HASH_ID == \
                TABLES.ZP_USER_IMAGE_RENDER_HASH.ID,
                TABLES.ZP_DUNE_USER_IMAGE_RENDER_HASH_XREF.RENDER_DATETIME >= \
            ...
            ZP_USER_IMAGE_RENDER_HASH_ID = None
        else:
            ZP_USER_IMAGE_RENDER_HASH_ID = ZP_USER_IMAGE_RENDER_HASH.ID
        session.close()
        return ZP_USER_IMAGE_RENDER_HASH_ID

    def set_dune_image_hash(self, **kwargs):
        session = self.Session()
        try:
            ZP_DUNE_USER_IMAGE_RENDER_HASH_XREF = session.query(
                TABLES.ZP_DUNE_USER_IMAGE_RENDER_HASH_XREF
            ).filter(
                TABLES.ZP_DUNE_USER_IMAGE_RENDER_HASH_XREF.ZP_DUNE_ID == kwargs['ZP_DUNE_ID'],
                TABLES.ZP_DUNE_USER_IMAGE_RENDER_HASH_XREF.ZP_USER_IMAGE_RENDER_HASH_ID == kwargs[
                    'ZP_USER_IMAGE_RENDER_HASH_ID']
            ).one()
        except orm.exc.NoResultFound:
            log.debug(('Adding ZP_DUNE_ID = {0},'
                       ' ZP_USER_IMAGE_RENDER_HASH_ID = {1} to'
                       ' ZP_DUNE_USER_IMAGE_RENDER_HASH_XREF').format(kwargs['ZP_DUNE_ID'],
                                                                      kwargs['ZP_USER_IMAGE_RENDER_HASH_ID']))
            add_ZP_DUNE_USER_IMAGE_RENDER_HASH_XREF = TABLES.ZP_DUNE_USER_IMAGE_RENDER_HASH_XREF(
                ZP_DUNE_ID=kwargs['ZP_DUNE_ID'],
                ZP_USER_IMAGE_RENDER_HASH_ID=kwargs['ZP_USER_IMAGE_RENDER_HASH_ID'],
                RENDER_DATETIME=date_time(),
                EXT=kwargs['image_extension']
            )
            session.add(add_ZP_DUNE_USER_IMAGE_RENDER_HASH_XREF)
            commit(session)
        else:
            ZP_DUNE_USER_IMAGE_RENDER_HASH_XREF.RENDER_DATETIME = date_time()
            ZP_DUNE_USER_IMAGE_RENDER_HASH_XREF.EXT = kwargs['image_extension']
            commit(session)
        session.close()
    ...
    def set_image_hash(self, **kwargs):
        """Set the hash used in the filename in the db
            ...
            Args:
                | ZP_USER_ID (int): the tv id
                | ZP_USER_IMAGE_TYPE_ID (int): tv image type (icon/synopsis) id
        ...
        try:
            ...
            log.debug(('Adding ZP_USER_ID = {0},'
                       ' image_ident_md5 = {1}').format(kwargs['ZP_USER_ID'],
                                                        kwargs['image_ident_md5']))
            add_ZP_USER_IMAGE_RENDER_HASH = TABLES.ZP_USER_IMAGE_RENDER_HASH(
                ZP_USER_ID=kwargs['ZP_USER_ID'],
                ZP_IMAGE_SUB_TYPE=kwargs['ZP_IMAGE_SUB_TYPE'],
                ZP_USER_IMAGE_TYPE_ID=kwargs['zp_image_type_id'],
                HASH=kwargs['image_ident_md5'],
                TEMPLATE_NAME=kwargs['template_name'],
                # EXT=kwargs['image_extension']
            )
            session.add(add_ZP_USER_IMAGE_RENDER_HASH)
            commit(session)
            kwargs['ZP_USER_IMAGE_RENDER_HASH_ID'] = add_ZP_USER_IMAGE_RENDER_HASH.ID
        else:
            kwargs['ZP_USER_IMAGE_RENDER_HASH_ID'] = ZP_USER_IMAGE_RENDER_HASH.ID
            # ZP_USER_IMAGE_RENDER_HASH.EXT = kwargs['image_extension']
            commit(session)

            log.debug(('ZP_USER_ID: {0},'
                       ' image_ident_md5: {1} allready in ZP_USER_IMAGE_RENDER_HASH'.format(kwargs['ZP_USER_ID'],
                                                                                            kwargs['image_ident_md5'])))
        session.close()
        ...
        for eapi in self.eapi_plugins_access_list:
            ...
            try:
                zp_eapi_eid = session.query(
                    TABLES.ZP_USER_EAPI_EID).filter(
                    TABLES.ZP_USER_EAPI_EID.ZP_USER_ID == kwargs['ZP_USER_ID'],
                    TABLES.ZP_USER_EAPI_EID.ZP_EAPI_ID == self.eapi_dict[eapi]).one().ZP_EAPI_EID
            except orm.exc.NoResultFound as e:
                log.warning('No resluts for ZP_USER_ID: {0} in ZP_USER_EAPI_EID'.format(
                    kwargs['ZP_USER_ID']))
            else:
    ...
    def get_resized_raw_image(self, speical_image_type, width, height, **kwargs):
        ...
        try:
            zp_film_raw_image_xref = session.query(TABLES.ZP_USER_RAW_IMAGE).filter(
                TABLES.ZP_USER_RAW_IMAGE.ID == TABLES.ZP_USER_FILM_ENTITY_XREF.ZP_USER_ENTITY_ID,
                TABLES.ZP_USER_FILM_ENTITY_XREF.ZP_USER_ID == kwargs['ZP_USER_ID'],
                TABLES.ZP_USER_FILM_ENTITY_XREF.ZP_USER_ID == kwargs['ZP_ENTITY_ID'],
                TABLES.ZP_USER_FILM_ENTITY_XREF.ZP_USER_ENTITY_TYPE_ID == speical_image_type_entity_type_dict[
                    speical_image_type]
            ).one()
        except orm.exc.NoResultFound:
            log.warning('could not find a entry in ZP_USER_RAW_IMAGE where ZP_USER_ID %s,'
                        ' ZP_USER_ID %s, ZP_USER_ENTITY_TYPE_ID %s (speical_image_type %s)',
            ...
            log.debug('Found a entry in ZP_USER_FILM_ENTITY_XREF where ZP_USER_ID %s,'
                        ' ZP_USER_ID %s, ZP_USER_ENTITY_TYPE_ID %s (speical_image_type %s) with id %s',
                        kwargs['ZP_USER_ID'], kwargs['ZP_ENTITY_ID'], speical_image_type_entity_type_dict[
