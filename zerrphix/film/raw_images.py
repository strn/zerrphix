...
class RawImages(RawImagesBase):
    ...
    def __init__(self, **kwargs):
        ...
        #self.eapi_eid_from_zp_film_id = MethodType(eapi_eid_from_zp_film_id, self)
    ...
    def get_film_missing_raw_eapi_image_no_retry(self, zp_film_id, eapi, zp_lang_id):
        session = self.Session()
        return_list = []
            ...
            qry_film_missing_raw_eapi_image = session.query(
                ...
                (~TABLES.ZP_FILM.ID.in_(
                    session.query(TABLES.ZP_FILM_RAW_IMAGE.ZP_ENTITY_ID).filter(
                        TABLES.ZP_FILM_RAW_IMAGE.ZP_ENTITY_TYPE_ID == 1,
                        TABLES.ZP_FILM_RAW_IMAGE.ZP_EAPI_ID == self.eapi_dict[eapi]
                    )) |
                ~TABLES.ZP_FILM.ID.in_(
                    session.query(TABLES.ZP_FILM_RAW_IMAGE.ZP_ENTITY_ID).filter(
                        TABLES.ZP_FILM_RAW_IMAGE.ZP_ENTITY_TYPE_ID == 2,
                        TABLES.ZP_FILM_RAW_IMAGE.ZP_EAPI_ID == self.eapi_dict[eapi]
                    ))),
                TABLES.ZP_FILM.ID.in_(
                    session.query(
        ...
        if qry_film_missing_raw_eapi_image_count > 0:
            film_missing_raw_eapi_image = qry_film_missing_raw_eapi_image.order_by(TABLES.ZP_FILM.ID.desc()).limit(1000)
            session.close()
            for film in film_missing_raw_eapi_image:
                return_list.append(film.ID)
        return return_list

    def get_film_missing_raw_eapi_image_retry(self, zp_film_id, eapi, zp_lang_id):
        session = self.Session()
        return_dict = {}
        ...
        if qry_film_missing_raw_eapi_image_count > 0:
            film_missing_raw_eapi_image = qry_film_missing_raw_eapi_image.order_by(TABLES.ZP_FILM.ID.desc()).limit(10000)
            session.close()
    ...
    def check_missing_eapi_raw_images(self, zp_film_id, eapi, zp_lang_id):
        session = self.Session()
        try:
            session.query(
                TABLES.ZP_FILM).filter(
    ...
    def check_film_for_image_type(self, zp_film_id, eapi, zp_lang_id, image_type_id):
        session = self.Session()
        try:
    ...
    def acquire_lang_images(self, zp_lang_id):
        ...
        if isinstance(max_film_id, int):
            for eapi in self.eapi_film_plugins_access_list:
                if hasattr(getattr(self, eapi), 'get_film_raw_images'):
                    film_processing_complete = False
                    zp_film_id = max_film_id + 1
                    while film_processing_complete is False:
                        log.debug('start get_film_missing_raw_eapi_image_no_retry')
                        zp_film_list = self.get_film_missing_raw_eapi_image_no_retry(zp_film_id, eapi, zp_lang_id)
                        log.debug('end get_film_missing_raw_eapi_image_no_retry %s', zp_film_list)
                        if zp_film_list:
                            for zp_film_list_id in zp_film_list:
                                zp_film_id = zp_film_list_id
                                self.set_current_library_process_desc_detail(self.library_config_dict['id'],
                                                             19,
                                                             'LANG: %s, Eapi: %s, Film: %s/%s' % (
                                                                                 zp_lang_id, eapi,
                                                                                 zp_film_id, max_film_id
                                                                             ))
                                zp_film_eapi_eid = self.eapi_eid_from_zp_film_id(self.eapi_dict[eapi], zp_film_id)
                                if zp_film_eapi_eid is not None:
                                    image_types = copy.deepcopy(self.image_types_dict)
                                    for image_type_key in self.image_types_keys:
                                        if self.check_film_for_image_type(zp_film_id, eapi, zp_lang_id,
                                                                     image_types[image_type_key]['id']) is True:
                                            del image_types[image_type_key]
                                    film_raw_image_root_path = os.path.join(
                                        self.library_config_dict['downloaded_images_library_root_path'], str(zp_film_id))
                                    if make_dir(film_raw_image_root_path):
                                        self.acquire_film_eapi_raw_images(zp_film_id, zp_lang_id,
                                                                          eapi, zp_film_eapi_eid,
                                                                          image_types,
                                                                          film_raw_image_root_path)
                                        if self.check_missing_eapi_raw_images(zp_film_id, eapi, zp_lang_id) is True:
                                            self.set_retry(1, 8, zp_film_id, self.eapi_dict[eapi], zp_lang_id)
                                            for image_type_key in self.image_types_keys:
                                                if image_type_key in image_types:
                                                    if self.check_film_for_image_type(zp_film_id, eapi, None,
                                                                                      image_types[image_type_key][
                                                                                          'id']) is True:
                                                        del image_types[image_type_key]
                                            if image_types:
                                                self.acquire_film_eapi_raw_images(zp_film_id, None,
                                                                              eapi, zp_film_eapi_eid,
                                                                              image_types,
                                                                              film_raw_image_root_path)
                                    else:
            ...
            if self.check_can_retry(1) is True:
                ...
                for eapi in self.eapi_film_plugins_access_list:
                    if hasattr(getattr(self, eapi), 'get_film_raw_images'):
                        ...
                        zp_film_id = max_film_id + 1
                        while film_processing_complete is False:
                            log.debug('start get_film_missing_raw_eapi_image_retry')
                            zp_film_dict = self.get_film_missing_raw_eapi_image_retry(zp_film_id, eapi, zp_lang_id)
                            log.debug('end get_film_missing_raw_eapi_image_retry %s', zp_film_list)
                            if zp_film_dict:
                                # we need the ids in deceding order but dict keys can be in any order
                                for zp_film_dict_id in reversed(sorted(zp_film_dict)):
                                    log.debug('retry processing zp_film_id %s for eapi %s', zp_film_id, eapi)
                                    zp_film_id = zp_film_dict_id
                                    if zp_film_dict[zp_film_id]['datetime'] + timedelta(
                                        days=zp_film_dict[zp_film_id]['delay']) <= datetime.now():
                                        log.debug('dt %s, plus %s is %s which is less than than now %s',
                                                  zp_film_dict[zp_film_id]['datetime'],
                                                  zp_film_dict[zp_film_id]['delay'],
                                                  zp_film_dict[zp_film_id]['datetime'] + timedelta(
                                                      days=zp_film_dict[zp_film_id]['delay']), datetime.now())
                                        self.set_current_library_process_desc_detail(self.library_config_dict['id'],
                                                             19,
                                        ...
                                        self.set_current_library_process_desc_detail(self.library_config_dict['id'],
                                                             19,
                                                             'LANG: %s, Eapi: %s, Film: %s/%s. Not Retrying' % (
                                                                                 zp_lang_id, eapi,
                                                                                 zp_film_id, max_film_id
                                                                             ))
    ...
    def acquire_film_eapi_raw_images(self, zp_film_id, zp_lang_id, eapi, zp_film_eapi_eid, image_types,
                                     film_raw_image_root_path):
        if hasattr(getattr(self, eapi), 'get_film_raw_images'):
            if zp_lang_id is None:
                iso_639_part1 = None
            ...
            if (iso_639_part1 is not None and zp_lang_id is not None) or \
                ...
                eapi_raw_iamges_dict = getattr(getattr(self, eapi), 'get_film_raw_images')(
                    zp_film_eapi_eid, iso_639_part1)
            ...
            log.warning('eapi %s, does not have function get_film_raw_images', eapi)

    def set_acquired_eapi_image(self, image_type_id, eapi, zp_film_id, zp_lang_id, image_reference,
                                new_film_image_filename):
        session = self.Session()
        try:
            ...
            add_zp_film_raw_image = TABLES.ZP_FILM_RAW_IMAGE(ZP_ENTITY_TYPE_ID = image_type_id,
            ...
            log.error('there is allready an extry for ZP_FILM_RAW_IMAGE with ZP_ENTITY_TYPE_ID %s'
                      ' ZP_ENTITY_ID %s, ZP_EAPI_ID %s, ZP_LANG_ID %s of ID %s', image_type_id, zp_film_id,
            self.eapi_dict[eapi], zp_lang_id, zp_film_raw_image.ID)
        session.close()
