...
from zerrphix.specials import tv_specials
...
class Artwork(ArtworkBase):
    ...
    def __init__(self, **kwargs):
        ...
        """
        self.smbcon = None
        self.specials = tv_specials.Specials(self.Session, self.library_config_dict['name'])
        ...
        self.image_type_dict = {'synopsis': 1, 'icon': 2, 'poster': 3}
    ...
    def get_entity_processing_dict(self, **kwargs):
        ...
        qry_tvs_with_eid = session.query(
            ...
            TABLES.ZP_TV.ID < kwargs['ZP_TV_ID'],
            TABLES.ZP_TV.ID.in_(
                session.query(
            ...
            ~TABLES.ZP_TV.ID.in_(
                session.query(
                    ...
                    TABLES.ZP_TV_IMAGE_RENDER_HASH.ZP_TV_IMAGE_TYPE_ID == kwargs['zp_image_type_id'],
                    TABLES.ZP_TV_IMAGE_RENDER_HASH.ID == \
                    TABLES.ZP_USER_TV_IMAGE_RENDER_HASH_XREF.ZP_TV_IMAGE_RENDER_HASH_ID,
                    ##

                    # TABLES.ZP_TV_IMAGE_RENDER_HASH.TEMPLATE_NAME == kwargs['template_name'],
                    # TABLES.ZP_TV_IMAGE_RENDER_HASH.TEMPLATE_VERSION == kwargs['template_version'],
                    TABLES.ZP_TV_IMAGE_RENDER_HASH.ZP_TEMPLATE_ID == kwargs['zp_template_id'],
                    ...
                    TABLES.ZP_TV_IMAGE_RENDER_HASH.ZP_IMAGE_SUB_TYPE == kwargs['ZP_IMAGE_SUB_TYPE']))
        )
        qry_tvs_with_eid_count = qry_tvs_with_eid.count()
        # print('qry_tvs_with_eid_count', qry_tvs_with_eid_count)
        log.debug('qry_tvs_with_eid_count: {0}. ZP_DUNE_ID: {1}, ZP_USER_ID:{2}'.format(
            qry_tvs_with_eid_count,
            kwargs['ZP_DUNE_ID'],
            kwargs['ZP_USER_ID']))
        if qry_tvs_with_eid_count > 0:
            tvs = qry_tvs_with_eid.order_by(TABLES.ZP_TV.ID.desc()).limit(100)
            # session.expunge(tvs)
            # session.flush()
            session.close()
            for tv in tvs:
    ...
    def porcess_entity(self, **kwargs):
        """Process tvs
            ...
            Args:
                ...
                | dune_render_store_dict (dict): {'render_root_tv_dir': '/path/to/render_root_tv_dir',
                |									'type': 'local'}

            Attributes:
                | tvs_processed (bool): Used to end while loop when \
                all tvs have been processed
s
        """
        #kwargs['ZP_TV_IMAGE_TYPE_ID'] = self.image_type_dict[kwargs['image_type']]
        allowed_image_type_list = {'icon', 'synopsis', 'poster', 'backdrop'}
        if kwargs['image_type'] in allowed_image_type_list:
            session = self.Session()
            max_tv_id = session.query(func.max(TABLES.ZP_TV.ID)).one()[0]
            session.close()
            if isinstance(max_tv_id, int):
                for ZP_IMAGE_SUB_TYPE in kwargs['icon_sub_type_require_list']:
                    ...
                    kwargs['ZP_TV_ID'] = ZP_TV_ID
                    entities_processed = False
                    while entities_processed == False:
                        ...
                        if entity_processing_list:
                            for ID in entity_processing_list:
                                ...
                                self.set_current_library_process_desc_detail(
                                    ...
                                    17,
                                    'User %s, TV %s/%s, Image_type %s, sub_type %s, Dune %s' % (
                                ...
                                if image_ident is not None:
                                    ...
                                    kwargs['ZP_TV_IMAGE_RENDER_HASH_ID'] = self.get_image_hash(**kwargs)
                                    if kwargs['ZP_TV_IMAGE_RENDER_HASH_ID'] is not None:
            ...
            log.warning('''kwargs['image_type'] %s not in allowed_image_type_list %s''',
                      kwargs['image_type'], allowed_image_type_list)
        ...
        if image_save_folder_exists:
            ...
            for ZP_TV_SPECIAL_TYPE_ID in sorted(specials_dict):
                ...
                if ZP_TV_SPECIAL_TYPE_ID in [2]:
                    ...
                    #print(ZP_TV_SPECIAL_TYPE_ID, specials_dict[ZP_TV_SPECIAL_TYPE_ID])
            ...
            log.debug(
                'rendering image for ZP_TV_ID: {0}, ZP_DUNE_ID: {1}, ZP_USER_ID: {2}, ZP_IMAGE_SUB_TYPE: {3},'
                ' self.image_type_dict[image_type]: {4}, image_ident_md5: {5},'
                ' template_name: {6}, template_version: {7}'.format(kwargs['ZP_TV_ID'],
    ...
    def get_image_hash(self, **kwargs):
        session = self.Session()
        try:
            ZP_TV_IMAGE_RENDER_HASH = session.query(TABLES.ZP_TV_IMAGE_RENDER_HASH).filter(
                TABLES.ZP_TV_IMAGE_RENDER_HASH.ZP_TV_ID == kwargs['ZP_TV_ID'],
                TABLES.ZP_TV_IMAGE_RENDER_HASH.ZP_IMAGE_SUB_TYPE == kwargs['ZP_IMAGE_SUB_TYPE'],
                TABLES.ZP_TV_IMAGE_RENDER_HASH.ZP_TV_IMAGE_TYPE_ID == kwargs['zp_image_type_id'],
                TABLES.ZP_TV_IMAGE_RENDER_HASH.HASH == kwargs['image_ident_md5'],
                ##
                ...
                TABLES.ZP_TV_IMAGE_RENDER_HASH.ID,
                TABLES.ZP_DUNE_TV_IMAGE_RENDER_HASH_XREF.RENDER_DATETIME >= \
                TABLES.ZP_TV.LAST_EDIT_DATETIME
            ).one()
        except orm.exc.NoResultFound:
            ZP_TV_IMAGE_RENDER_HASH_ID = None
        else:
            ZP_TV_IMAGE_RENDER_HASH_ID = ZP_TV_IMAGE_RENDER_HASH.ID
        session.close()
        return ZP_TV_IMAGE_RENDER_HASH_ID

    def set_dune_image_hash(self, **kwargs):
        session = self.Session()
        try:
            ZP_DUNE_TV_IMAGE_RENDER_HASH_XREF = session.query(
                TABLES.ZP_DUNE_TV_IMAGE_RENDER_HASH_XREF
            ).filter(
                TABLES.ZP_DUNE_TV_IMAGE_RENDER_HASH_XREF.ZP_DUNE_ID == kwargs['ZP_DUNE_ID'],
                TABLES.ZP_DUNE_TV_IMAGE_RENDER_HASH_XREF.ZP_TV_IMAGE_RENDER_HASH_ID == kwargs[
                    'ZP_TV_IMAGE_RENDER_HASH_ID']
            ).one()
        except orm.exc.NoResultFound:
            log.debug(('Adding ZP_DUNE_ID = {0},'
                       ' ZP_TV_IMAGE_RENDER_HASH_ID = {1} to'
                       ' ZP_DUNE_TV_IMAGE_RENDER_HASH_XREF').format(kwargs['ZP_DUNE_ID'],
                                                                    kwargs['ZP_TV_IMAGE_RENDER_HASH_ID']))
            add_ZP_DUNE_TV_IMAGE_RENDER_HASH_XREF = TABLES.ZP_DUNE_TV_IMAGE_RENDER_HASH_XREF(
                ZP_DUNE_ID=kwargs['ZP_DUNE_ID'],
                ZP_TV_IMAGE_RENDER_HASH_ID=kwargs['ZP_TV_IMAGE_RENDER_HASH_ID'],
                RENDER_DATETIME=date_time(),
                EXT=kwargs['image_extension']
            )
            session.add(add_ZP_DUNE_TV_IMAGE_RENDER_HASH_XREF)
            commit(session)
        else:
            ZP_DUNE_TV_IMAGE_RENDER_HASH_XREF.RENDER_DATETIME = date_time()
            ZP_DUNE_TV_IMAGE_RENDER_HASH_XREF.EXT = kwargs['image_extension']
            commit(session)
        session.close()
    ...
    def set_user_image_hash(self, **kwargs):
        """Set the hash used for the user in the filename in the db
            ...
            Raises:
                Exception: if cannot find the hash in ZP_TV_IMAGE_RENDER_HASH as it should be in there

        """
        session = self.Session()
        try:
            ZP_USER_TV_IMAGE_RENDER_HASH_XREF = session.query(
                TABLES.ZP_USER_TV_IMAGE_RENDER_HASH_XREF).filter(
                TABLES.ZP_USER_TV_IMAGE_RENDER_HASH_XREF.ZP_TV_ID == kwargs['ZP_TV_ID'],
                TABLES.ZP_USER_TV_IMAGE_RENDER_HASH_XREF.ZP_USER_ID == kwargs['ZP_USER_ID'],
                TABLES.ZP_USER_TV_IMAGE_RENDER_HASH_XREF.ZP_TV_IMAGE_TYPE_ID == kwargs['zp_image_type_id'],
                TABLES.ZP_USER_TV_IMAGE_RENDER_HASH_XREF.ZP_IMAGE_SUB_TYPE == kwargs['ZP_IMAGE_SUB_TYPE']
            ).one()
        except orm.exc.NoResultFound as e:
            log.debug(('Adding ZP_TV_ID = {0}').format(kwargs['ZP_TV_ID']))
            add_ZP_USER_TV_IMAGE_RENDER_HASH_XREF = TABLES.ZP_USER_TV_IMAGE_RENDER_HASH_XREF(
                ...
                ZP_TV_IMAGE_RENDER_HASH_ID=kwargs['ZP_TV_IMAGE_RENDER_HASH_ID'])
            session.add(add_ZP_USER_TV_IMAGE_RENDER_HASH_XREF)
            commit(session)
        else:
            if ZP_USER_TV_IMAGE_RENDER_HASH_XREF.ZP_TV_IMAGE_RENDER_HASH_ID != kwargs['ZP_TV_IMAGE_RENDER_HASH_ID']:
                ZP_USER_TV_IMAGE_RENDER_HASH_XREF.ZP_TV_IMAGE_RENDER_HASH_ID = kwargs['ZP_TV_IMAGE_RENDER_HASH_ID']
                commit(session)
        session.close()
        ...
        try:
            ...
            log.debug(('Adding ZP_TV_ID = {0},'
                       ' image_ident_md5 = {1}').format(kwargs['ZP_TV_ID'],
                                                        kwargs['image_ident_md5']))
            add_ZP_TV_IMAGE_RENDER_HASH = TABLES.ZP_TV_IMAGE_RENDER_HASH(
                ZP_TV_ID=kwargs['ZP_TV_ID'],
                ZP_IMAGE_SUB_TYPE=kwargs['ZP_IMAGE_SUB_TYPE'],
                ZP_TV_IMAGE_TYPE_ID=kwargs['zp_image_type_id'],
            ...
            session.add(add_ZP_TV_IMAGE_RENDER_HASH)
            commit(session)
            kwargs['ZP_TV_IMAGE_RENDER_HASH_ID'] = add_ZP_TV_IMAGE_RENDER_HASH.ID
        else:
            kwargs['ZP_TV_IMAGE_RENDER_HASH_ID'] = ZP_TV_IMAGE_RENDER_HASH.ID
            # ZP_TV_IMAGE_RENDER_HASH.EXT = kwargs['image_extension']
            commit(session)
        ...
        for eapi in self.eapi_plugins_access_list:
            ...
            try:
                zp_eapi_eid = session.query(
                    TABLES.ZP_TV_EAPI_EID).filter(
                    TABLES.ZP_TV_EAPI_EID.ZP_TV_ID == kwargs['ZP_TV_ID'],
                    TABLES.ZP_TV_EAPI_EID.ZP_EAPI_ID == self.eapi_dict[eapi]).one().ZP_EAPI_EID
            except orm.exc.NoResultFound as e:
                log.warning('No resluts for ZP_TV_ID: {0} in ZP_TV_EAPI_EID'.format(
                    kwargs['ZP_TV_ID']))
            else:
    ...
    def get_resized_raw_image(self, speical_image_type, width, height, **kwargs):
        ...
        speical_image_type_entity_type_dict = {'poster': 3, 'backdrop':4, 'banner': 5}
        try:
            zp_film_raw_image_xref = session.query(TABLES.ZP_TV_RAW_IMAGE).filter(
                ...
                TABLES.ZP_USER_TV_ENTITY_XREF.ZP_TV_ID == kwargs['ZP_ENTITY_ID'],
            ...
            log.debug('Found a entry in ZP_USER_TV_ENTITY_XREF where ZP_USER_ID %s,'
